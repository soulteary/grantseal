...
```
  ...
  cmd/license-tool  ──调用──▶  internal/issuer          |   应用 ──import──▶ pkg/license
      ...
      revoke-list                规范化 payload         |                     Manager（文件 I/O、缓存）
                              ...
                              revoked.json                                （fail-closed，绝不 panic）
                                                        |
...
失败路径都返回稳定的 `LICENSE_*` 错误码，且对非法输入绝不 panic。

## 撤销与离线新鲜度

- 撤销列表的认证方式与许可完全一致：规范化字节、Ed25519 签名，客户端针对同一
  `KeyRing` 验证。其签名体内的 `key_id` 必须与信封 `key_id` 一致，schema 必须为 `1`，
  条目数受 `MaxRevokedIDs` 限制。
- **离线新鲜度限制。** 客户端只强制执行它当前持有的撤销列表。没有新的签名列表，它就
  无法得知更新的撤销。`issued_at` 时间戳可供运维推断陈旧程度，但目前**不是**自动的
  新鲜度强制机制。
...
- **撤销列表新鲜度强制**（例如最大陈旧时长 / “必须新于某时间”的策略）。*Roadmap。*
...
3. **规范化**：丢弃空值，按 `(category, value)` 确定性排序，前缀加产品命名空间与一个
   NUL 分隔符，再用 `\n` 连接 `category=value` 行。
4. **散列**：默认 SHA-256；提供密钥时用 HMAC-SHA256（`ComputeHMAC`）。输出前缀为
   `sha256:`。
