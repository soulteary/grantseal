...
- 朴素时钟回拨通过完整性保护的时间高水位线被**检测**，但无法防住 root/admin 级攻击者。状态 HMAC 是**针对普通文件修改的防篡改（tamper-evident），并非安全的密钥存储**：内置密钥可从客户端二进制中逆向还原，因此对于（不在防护范围内的）本地高权限攻击者而言，它在密码学上并非不可伪造。
...
- 撤销状态存储为**单进程写者**：`FileRevocationStateStore` 以按路径锁协调同一进程内的实例，但不获取操作系统级文件锁，因此无法保护分处不同进程写同一状态文件的并发写者。请采用三种安全部署形态之一——（1）使用文件存储的**单一写者进程**（可选 `NewFileRevocationStateStoreExclusive`，在进程内重复构造写者时 fail-closed，并由 `Close` 释放）；（2）面向多进程/多实例的**自定义原子后端**（SQLite/Redis/RDBMS，跨进程可串行化的 compare-and-set）；（3）**只读副本 + 单一写者**。该进程内护栏无法察觉第二个操作系统进程；详见根目录 [`SECURITY.md`](../../SECURITY.md#revocation-state-store-concurrency--deployment)。
