...
if err := res.RequireFeature("api"); err != nil {
    // license.CodeOf(err) == license.CodeFeatureUnavailable
...
| "Is this edition/feature allowed?" | Edition defaults unioned with `features`; `RequireFeature` | `LICENSE_FEATURE_UNAVAILABLE` |
...
- **Coverage:** `go test ./... -covermode=atomic` reports the total statement
  coverage published from a single machine-readable source of truth
  ([`.github/go-test-report.json`](./.github/go-test-report.json)); the CI gate
  is the floor of that measured total. The exact percentage and per-package
  breakdown are generated into [`docs/enUS/quality.md`](./docs/enUS/quality.md)
  by `ci-recipes grantseal generate-quality-docs` (do not hand-edit), and the Coverage
  badge above is generated by CI from the same run.
...
if err := res.RequireFeature("api"); err != nil {
    // license.CodeOf(err) == license.CodeFeatureUnavailable
    // （CodeFeatureDenied 是同一 wire code 的 Go 别名）
}
...
| “这个版本/功能是否被允许？” | edition 默认功能与 `features` 取并集；`RequireFeature` | `LICENSE_FEATURE_UNAVAILABLE` |
...
- **覆盖率：** `go test ./... -covermode=atomic` 的总语句覆盖率来自唯一机器可读事实来源
  （[`.github/go-test-report.json`](./.github/go-test-report.json)），CI 门禁取该实测
  总覆盖率的向下取整。具体百分比与分包细节由 `ci-recipes grantseal generate-quality-docs`
  生成到 [`docs/zhCN/quality.md`](./docs/zhCN/quality.md)（请勿手工编辑），顶部
  Coverage 徽章由 CI 基于同一次运行生成。
